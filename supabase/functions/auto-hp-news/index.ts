...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const systemPrompt =
      "あなたはメンズエステ『全力エステ 仙台』の新着ニュースを書くプロの編集者です。読み手が飽きない、短く歯切れの良い文章を書いてください。\n" +
      "【厳守ルール】\n" +
      "・文体は必ず敬語（です・ます調）。『〜だ』『〜である』『〜できる』『〜しない』などの常体は禁止。お客様向けの丁寧で上品なトーンにする（ひとことニュース・本文とも）。\n" +
      "・料金/割引/出勤など具体情報は『参照データ』にある事実のみ使用。記載のない金額・割引名・キャスト名・日時は創作禁止。参照データが空なら具体値・固有名は避ける。\n" +
      "・冗長・単調はNG。1文を短く、テンポよく。同じ言い回しの繰り返しを避ける。\n" +
      "・視覚的にメリハリを付ける: 重要語（割引名・金額・キャスト名・日付・『本日』など）は **太字** で強調する。話題の切れ目に短い小見出しを付ける場合は行頭に ◆ を付ける。\n" +
      "・上記(** と ◆)以外のMarkdown（#見出し・表|・リンク・コード・箇条書き記号）やハッシュタグは使わない。\n" +
      "【出力形式（この形を厳守）】\n" +
      "1行目: 記事タイトル（15〜22字・記号や絵文字なし・太字記号なし）\n" +
      "2行目: 空行\n" +
      "3行目: 「💡ひとことニュース：」に続けて、メンズエステ業界や季節の話題を1文で軽快に（30〜45字。トレンド・雑学・季節ネタなど。店舗固有の数値は入れない）\n" +
      "4行目: 空行\n" +
      "5行目以降: 本文（合計120〜200字）。短い段落2つ程度、または ◆小見出し＋短文。割引や出勤は要点だけ簡潔に、重要語は **太字** で強調。絵文字は全体で2個まで。";

    const factsBlock = facts
      ? `\n\n===== 参照データ（この事実のみ使用可。創作禁止）=====\n${facts}\n===============================================\n`
      : "";
    const userPrompt =
      `本日(${ymd})の新着ニュースを、上記の出力形式で作成してください。文体は必ず敬語（です・ます調）です。「ひとことニュース」は業界・季節の軽い話題を1文で。本文は参照データの割引・出勤・料金の要点だけを短くまとめ、テンポよく読みやすくしてください。参照データに無い具体的な数値や固有名は使わないこと。${factsBlock}`;
    ...
    // force 再生成時は同slugの既存記事を置き換える
    if (force) await sb.from("hp_articles").delete().eq("slug", slug);
