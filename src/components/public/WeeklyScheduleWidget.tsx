    ...
    <section className="py-5 md:py-8" style={{ background: "linear-gradient(180deg, #2e2b29 0%, #3a3330 100%)" }}>
      <div className="container mx-auto max-w-6xl px-3 md:px-6">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-[10px] tracking-widest text-[#c49480] mb-0.5">WEEKLY SCHEDULE</p>
            <h2 className="text-base md:text-lg font-bold tracking-wider text-white">週間出勤予定</h2>
          </div>
          <Link
            to="/schedule"
            className="text-xs text-[#c49480] border border-[#c49480]/50 px-2.5 py-1 rounded hover:bg-[#c49480]/10 transition"
          >
            一覧を見る ▶
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-center text-[#9a8c88] py-8 text-sm">この日の出勤予定はありません</p>
        ) : (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
            {shifts.map((shift) => {
              const c = shift.casts;
              ...
              return (
                ...
                >
                  <div className="relative rounded overflow-hidden bg-[#2a2320]">
                    ...
                    <div className="aspect-[3/4]">
                      {c.photo ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-2xl text-[#c49480]/40">
                          {c.name.charAt(0)}
                        </div>
                      )}
                      {/* Time overlay */}
                      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent pt-4 pb-1 px-1">
                        <div className="flex items-center justify-center gap-0.5">
                          <Clock size={8} className="text-[#c49480] shrink-0" />
                  ...
                  <div className="mt-1 px-0.5">
                    ...
                    {stats && (
                      <p className="text-[9px] text-[#9a8c88] truncate leading-tight mt-0.5">{stats}</p>
                    )}
                  </div>
                </Link>
              );
